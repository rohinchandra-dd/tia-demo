...
def test_inventory_cache_race():
    if random.random() < 0.35:
        pytest.fail("Simulated inventory cache race condition")
...
def test_email_delivery_timing():
    if random.random() < 0.35:
        pytest.fail("Simulated SMTP timing flake")
...
def test_tracking_webhook_delay():
    if random.random() < 0.35:
        pytest.fail("Simulated carrier webhook delay")
...
def test_stock_level_threshold_jitter():
    if random.random() < 0.35:
        pytest.fail("Simulated stock threshold jitter")
